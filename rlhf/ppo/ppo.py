...
from transformers import HfArgumentParser, AutoTokenizer, AutoModelForSequenceClassification
...
class ScriptArguments:
    ...
    eval_every: Optional[int] = field(default=3)
...
# eval_model(ppo_trainer, eval_dataset, tokenizer, accelerator, script_args, rm_tokenizer, rm_gpu_id, reward_model, name, eval_generation_kwargs)
gold_rm_tokenizer = AutoTokenizer.from_pretrained('Ray2333/reward-model-Mistral-7B-instruct-Unified-Feedback', use_fast=False)
gold_reward_model = AutoModelForSequenceClassification.from_pretrained('Ray2333/reward-model-Mistral-7B-instruct-Unified-Feedback', num_labels=1, device_map=accelerator.local_process_index, torch_dtype=torch.bfloat16)
gold_rm_gpu_id = gpu_id

eval_model(ppo_trainer, eval_dataset, tokenizer, accelerator, script_args, rm_tokenizer, rm_gpu_id, reward_model, gold_reward_model, gold_rm_tokenizer, gold_rm_gpu_id, name, eval_generation_kwargs)
...
for epoch in range(epochs):
    ...
    for i, batch in enumerate(ppo_trainer.dataloader):
        ...
        with torch.no_grad():
            reward_tensors = [reward_model(x['input_ids'].to(rm_gpu_id)).logits[0].mean() for x in encoded_prompt_response] 
            print('reward_tensor', reward_tensors)
