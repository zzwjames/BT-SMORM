...
# Load datasets
print('dataset', script_args.dataset)
...
## load base model with 6 labels (1 for preference, 5 for regression)
...
# Define a multi-head wrapper to split the output logits.
class MultiHeadModel(nn.Module):
    ...
    def forward(self, input_ids, attention_mask=None, **kwargs):
        # Get the raw logits with shape (batch_size, 6)
        outputs = self.base_model(input_ids=input_ids, attention_mask=attention_mask, **kwargs)
        logits = outputs.logits
        # The first logit is the preference score; the rest are for regression.
        pref_score = logits[:, 0]          # shape: (batch_size,)
        reg_outputs = logits[:, 1:]          # shape: (batch_size, 5)
        # Return a tuple with both outputs.
        return (pref_score, reg_outputs)

# Wrap the base model.
model = MultiHeadModel(base_model)

if script_args.freeze_pretrained:
    # For frozen baseline, we replace the classifier with a trainable MLP.
    mlp_layer = nn.Sequential(
        nn.Linear(model.base_model.config.hidden_size, 1024, dtype=torch.bfloat16),  
        nn.ReLU(),
